...
const ClientTender = () => {
  ...
  useEffect(() => {
    const fetchTenders = async () => {
      try {
        ...
        // Fetch tenders for the current client
        const tenders = await tenderService.getTendersByClientId(currentUser.id);
        setUserTenders(tenders);
      } catch (err) {
  ...
  const closeTender = async (id) => {
    try {
      setIsLoading(true);
      // Update tender status to 'ended'
      const updatedTender = await tenderService.updateTender(id, { status: 'ended' });
